...
class Zerodha(KiteConnect):
    ...
    def basket_order_margins(self, params, consider_positions=True, mode=None):
        ...
        - `params` is a list of order dicts, each with keys like:
            exchange, tradingsymbol, transaction_type, variety, product,
            order_type, quantity, price, trigger_price
        - `consider_positions` – factor in existing positions (default True)
        - `mode` – "compact" returns only total margins; None returns full detail
        """
